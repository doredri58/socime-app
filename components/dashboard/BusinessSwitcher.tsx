...
export default function BusinessSwitcher() {
  ...
  return (
    <div ref={boxRef} style={{ position: 'relative' }}>
      ...
      >
        <i className="ti ti-building-store" style={{ fontSize: 14, color: '#9656FE', flexShrink: 0 }} />
        ...
        <div style={{
          ...
          {items.map(b => (
            <button key={b.id} onClick={() => switchTo(b.id)} style={{
              ...
              <i className="ti ti-building-store" style={{ fontSize: 14, color: b.id === activeId ? '#9656FE' : 'var(--dash-sidebar-label)' }} />
              <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.name}</span>
              {b.id === activeId && <i className="ti ti-check" style={{ fontSize: 14, color: '#9656FE' }} />}
            </button>
          ))}
          {canAdd && (

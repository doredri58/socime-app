    ...
    <div style={{ direction: 'rtl', paddingBottom: 60 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button onClick={handleSave} disabled={loading} style={{
          ...
          boxShadow: '0 4px 18px rgba(152,80,255,0.35)', opacity: loading ? 0.75 : 1,
          transition: 'all 0.2s',
        }}>
          {loading
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        ══════════════════════════════════════════════════════════ */}
        <SectionCard
          icon="ti-brain" iconBg="rgba(152,80,255,0.15)" iconColor={PURPLE2}
          title="איך SociMe מבינה אתכם"
          subtitle="סיכום AI של פרופיל העסק שלך — הבסיס לכל התוכן שנכתב"
          glow={PURPLE}
        >
          {/* ambient glow blobs */}
          <div style={{ position: 'absolute', top: -40, right: -40, width: 140, height: 140, borderRadius: '50%', background: 'rgba(152,80,255,0.10)', filter: 'blur(50px)', pointerEvents: 'none' }} />
          ...
          {systemPrompt ? (
            <>
              {editingPrompt ? (
                ...
                <div style={{
                  ...
                  background: 'rgba(152,80,255,0.07)', border: '1px solid rgba(152,80,255,0.18)',
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                ...
                <button onClick={handleRegen} disabled={regen} style={{
                  ...
                  boxShadow: '0 3px 12px rgba(152,80,255,0.3)', transition: 'all 0.18s',
                }}>
                  {regen
            ...
            <div style={{ padding: '24px', textAlign: 'center', borderRadius: 14, background: 'rgba(152,80,255,0.06)', border: '1px dashed rgba(152,80,255,0.25)' }}>
              <i className="ti ti-brain" style={{ fontSize: 28, color: 'rgba(152,80,255,0.4)', display: 'block', marginBottom: 8 }} />
            ...
            <Field label="תיאור העסק הכללי" icon="ti-file-description">
              <div style={{ position: 'relative' }}>
                <textarea
                  ...
                  onFocus={e => { (e.target as HTMLTextAreaElement).style.borderColor = `${PURPLE}80`; (e.target as HTMLTextAreaElement).style.boxShadow = '0 0 0 3px rgba(152,80,255,0.10)' }}
            ...
            <Field label="טון וסגנון דיבור" icon="ti-message-circle">
              <div>
                  ...
                  {TONES.map(tone => {
                    ...
                    return (
                      <button key={tone.id} onClick={() => toggleTone(tone.id)} style={{
                        ...
                        background: active ? 'rgba(152,80,255,0.18)' : 'rgba(255,255,255,0.05)',
                        border: `1px solid ${active ? 'rgba(190,86,255,0.45)' : 'rgba(255,255,255,0.09)'}`,
                        color: active ? PURPLE2 : 'rgba(255,255,255,0.45)',
                        display: 'flex', alignItems: 'center', gap: 6,
                        boxShadow: active ? '0 2px 10px rgba(152,80,255,0.18)' : 'none',
                        transition: 'all 0.18s',
                      }}>
                ...
                {selectedTones.length > 0 && (
                  <div style={{ marginTop: 12, padding: '10px 14px', borderRadius: 12, background: 'rgba(152,80,255,0.07)', border: '1px solid rgba(152,80,255,0.15)', fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-start', gap: 12, paddingTop: 4 }}>
          <button onClick={handleSave} disabled={loading} style={{
            ...
            boxShadow: '0 4px 22px rgba(152,80,255,0.4)', transition: 'all 0.2s',
          }}

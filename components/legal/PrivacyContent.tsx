...
// showAccountActions — פעולות "ייצוא/מחיקת חשבון" רלוונטיות רק למשתמש מחובר
// (בדשבורד). בעמוד הציבורי הן מוסתרות כי הן דורשות התחברות.
export default function PrivacyContent({ showAccountActions = true }: { showAccountActions?: boolean }) {
  ...
  return (
    <div style={{ direction: 'rtl', maxWidth: 860, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 32 }}>
        {SECTIONS.map((sec, i) => (
          <div key={i} style={{
            ...
            border: openSection === i ? '1px solid rgba(150,86,254,0.25)' : '1px solid rgba(255,255,255,0.09)',
      ...
      {/* Actions — only for logged-in users (dashboard) */}
      {showAccountActions && (
        <>

...
const SECTIONS = [
  ...
  },
  {
    num: '5',
    title: 'שימוש מותר',
    body: 'מותר: שימוש לצרכים עסקיים לגיטימיים, שיתוף תוכן מקורי, ניהול עמודים שבבעלותכם. אסור: spam, תוכן פוגעני, הטרדה, הפצת מידע כוזב, פרסום ללא הסכמה, הפרת תנאי שירות של הפלטפורמות (פייסבוק, אינסטגרם, טיקטוק). הפרת הכללים עלולה לגרור השעיית חשבון.',
  },
  {
    num: '6',
    title: 'תשלום, חידוש וביטול',
    body: 'המנוי נגבה מראש לכל תקופת חיוב (חודשי או שנתי) ומתחדש אוטומטית בסוף כל תקופה באמצעות אמצעי התשלום השמור, עד לביטול על ידכם. אחריות 14 יום: בתוך 14 הימים הראשונים מהרכישה הראשונה ניתן לקבל החזר כספי מלא ללא צורך בנימוק. לאחר מכן אין החזרים חלקיים, אך ניתן לבטל את החידוש בכל עת — השירות יישאר פעיל עד תום התקופה ששולמה. שדרוג מסלול נכנס לתוקף מיידית; הורדת מסלול בתחילת החיוב הבא. SociMe שומרת לעצמה את הזכות לשנות מחירים עם הודעה של 30 יום מראש. כל החיובים בשקלים חדשים (₪).',
  },
  {
    num: '7',
    title: 'קניין רוחני',
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
          <div style={{
            ...
            background: 'linear-gradient(135deg, rgba(150,86,254,0.2), rgba(190,86,254,0.08))',
            border: '1px solid rgba(150,86,254,0.3)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            <i className="ti ti-file-description" style={{ fontSize: 22, color: '#BE56FE' }} />
          </div>
          <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 20px' }}>
          {SECTIONS.map(s => (
            <a key={s.num} href={`#sec-${s.num}`} style={{
              ...
              <span style={{ color: '#9656FE', fontWeight: 700 }}>{s.num}.</span>
              {s.title}
            </a>
          ))}
        </div>
        ...
        {SECTIONS.map(s => (
          <div key={s.num} id={`sec-${s.num}`} style={{ ...GLASS, padding: '20px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 10 }}>
              <span style={{
                fontSize: 11, fontWeight: 900, color: '#9656FE',
      ...
      <div style={{ ...GLASS, padding: '22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <a href="mailto:legal@socime.co.il" style={{
            padding: '9px 18px', borderRadius: 10, fontSize: 12, fontWeight: 700,
            background: 'linear-gradient(135deg, #9656FE, #BE56FE)',
